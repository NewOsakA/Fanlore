...
class GiveAchievementView(FormView):
    ...
    def form_valid(self, form):
        achievement = get_object_or_404(Achievement, id=form.cleaned_data["achievement_id"])
        user = get_object_or_404(User, id=form.cleaned_data["user_id"])

        # Create the achievement
        UserAchievement.objects.create(user=user, achievement=achievement)
        messages.success(self.request, f"Achievement '{achievement.name}' given to {user.username}!")
